...
def load_tiff_dataset(path="CS271_final_data", img_size=(224, 224)):
    ...
    for folder in sorted(os.listdir(path)):
        ...
        for filename in os.listdir(folder_path):
            if (not filename.lower().endswith(("_f.tiff")) or filename.startswith("X")):
                continue
...
# 80:20 split
X_train_f, X_test_f, y_train_f, y_test_f = train_test_split(images, labels, test_size=0.20, random_state=42, stratify=labels)

X_train_f = X_train_f.astype("float32") / 255.0
X_test_f = X_test_f.astype("float32") / 255.0

print("Train size for class a:", len(X_train_f))
print("Test size for class a:", len(X_test_f))

y_train_f = y_train_f.astype(np.int32)
y_test_f = y_test_f.astype(np.int32)

print(X_train_f.dtype, X_train_f.shape)
print(type(X_train_f[0]))
print(y_train_f[:10])
print(type(y_train_f[0]))


def train_CNN_class_f(X_train, y_train, X_test, y_test):
...
accuracy, model, history = train_CNN_class_f(X_train_f, y_train_f, X_test_f, y_test_f)
print(f"Test accuracy: {accuracy * 100:.2f}%")
model.save('cnn_class_f_model.h5')

plt.plot(history.history['accuracy'], label = 'Training Accuracy')
plt.plot(history.history['val_accuracy'], label = 'Validation Accuracy')
plt.title('CNN: Model F Accuracy Graph')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
...
plt.title('CNN: Model F Loss Graph')
plt.xlabel('Epoch')
plt.ylabel('Loss')
