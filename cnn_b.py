...
def load_tiff_dataset(path="CS271_final_data", img_size=(224, 224)):
    ...
    for folder in sorted(os.listdir(path)):
        ...
        for filename in os.listdir(folder_path):
            if (not filename.lower().endswith(("_b.tiff")) or filename.startswith("X")):
                continue
...
print("Train size for class b:", len(X_train_b))
print("Test size for class b:", len(X_test_b))

y_train_b = y_train_b.astype(np.int32)
y_test_b = y_test_b.astype(np.int32)

print(X_train_b.dtype, X_train_b.shape)
print(type(X_train_b[0]))
print(y_train_b[:10])
print(type(y_train_b[0]))

def train_CNN_class_b(X_train, y_train, X_test, y_test):
    ...
    model = tf.keras.Sequential([
        ...
        #layers.Dropout(0.2),
        #layers.Dense(60, activation='softmax'), # output layer 
        #layers.Dropout(0.2), 
        layers.Dense(12, activation='softmax'), # output layer 
    ])
    
    optimizer_custom = optimizers.Adam(learning_rate=0.001)
...
accuracy, model, history = train_CNN_class_b(X_train_b, y_train_b, X_test_b, y_test_b)
print(f"Test accuracy: {accuracy * 100:.2f}%")
model.save('cnn_class_b_model.h5')

plt.plot(history.history['accuracy'], label='Training Accuracy')
plt.plot(history.history['val_accuracy'], label='Validation Accuracy')
plt.title('CNN for Sample B Model Accuracy')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
...
plt.title('CNN for Sample B Model Loss')
plt.xlabel('Epoch')
plt.ylabel('Loss')
